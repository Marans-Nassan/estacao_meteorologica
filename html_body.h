...
"<title>Observação Meteorológica</title>"
...
"</style></head><body>"
"<div class=\"container\"><h1 class=\"title\">💧 Observação Meteorológica</h1>"
"<div id=\"status\" class=\"status\">Conectando…</div>"
"<div class=\"sensor-data\">"
"<div class=\"sensor-box\"><div>Temp BMP</div><div id=\"temp-bmp\">-- °C</div></div>"
"<div class=\"sensor-box\"><div>Altitude</div><div id=\"altitude\">-- m</div></div>"
"<div class=\"sensor-box\"><div>Temp AHT</div><div id=\"temp-aht\">-- °C</div></div>"
...
"chart=new Chart(ctx,{type:'line',data:{labels:[],datasets:[{label:'Nível (%)',data:[],"
...
"if(val<d.min||val>d.max){cv.style.color='#ff6b6b';cv.innerHTML=val+'% <span>⚠</span>'}else cv.style.color='#2ecc71';"
"document.getElementById('temp-bmp').textContent=d.temp_bmp.toFixed(1)+' °C';"
"document.getElementById('altitude').textContent=d.altitude.toFixed(1)+' m';"
"document.getElementById('temp-aht').textContent=d.temp_aht.toFixed(1)+' °C';"
"document.getElementById('humidity').textContent=d.humidity.toFixed(1)+' %';"
"}catch{const st=document.getElementById('status');st.textContent='Falha';st.className='status error';"
"document.getElementById('current-value').textContent='--%'}}"
"async function loadSettings(){"
"try{const res=await fetch('/api/data'),d=await res.json();console.log('Configurações:', d);"
...
"  if(min>=max){fb.style.color='#e74c3c';fb.textContent='Min<Max';return}"
"  if(min<0||max>100||off<-100||off>100){fb.style.color='#e74c3c';fb.textContent='Valor inválido';return}"
"  try{"
"    fb.textContent='Salvando...';fb.style.color='#3498db';"
"    const r=await fetch('/api/limites',{"
"      method:'POST',"
"      headers:{'Content-Type':'application/x-www-form-urlencoded'},"
"      body:`min=${min}&max=${max}&offset=${off}`"
"    });"
"    if(!r.ok) throw 0;"
"    fb.textContent='Salvo';fb.style.color='#2ecc71';"
"    setTimeout(fetchData,500);"
"  }catch{"
"    fb.style.color='#e74c3c';fb.textContent='Erro ao salvar';"
"  }"
"});"
